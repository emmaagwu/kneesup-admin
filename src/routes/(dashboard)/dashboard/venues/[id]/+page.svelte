<script lang="ts">
  ...
  const sharedSpaces: Space[] = [
    { id: 's1', name: 'Conference Room', description: 'Cras pharetra mi tristique sapien vestibulum lobortis. Nam eget bibendum metus,', maxGuest: 25,   status: 'active',   pricingModel: 'Contact for Pricing' },
    { id: 's2', name: 'Exhibition Hall',  description: 'non dictum mauris. Nulla at tellus sagittis, viverra est a, bibendum metus....', maxGuest: 600,  status: 'active',   pricingModel: 'Payment per Guest' },
    { id: 's3', name: 'Auditorium',       description: 'in pretium orci vestibulum eget. Class aptent taciti sociosqu ad litora torquent', maxGuest: 1500, status: 'inactive', pricingModel: 'Payment per Hour' },
  ];
        ...
        <table class="w-full">
          <thead>
            <tr class="bg-[#fafafa] border-b border-[#f0f0f0]">
              ...
              <th class="text-left px-4 py-3 text-xs font-semibold text-[#6b7280]">Pricing Model</th>
          ...
          <tbody class="divide-y divide-[#f9fafb]">
            {#each venue.spaces as space}
              <tr class="hover:bg-[#fafafa] transition-colors cursor-pointer"
                  onclick={() => goto(`/dashboard/venues/${venue.id}/spaces/${space.id}`)}>
                <td class="px-5 py-4 text-sm font-semibold text-[#111827] whitespace-nowrap">{space.name}</td>
                <td class="px-4 py-4 text-sm text-[#6b7280] max-w-[240px]">
                ...
                <td class="px-4 py-4 relative" onclick={(e) => e.stopPropagation()}>
                  ...
                  {#if activeSpaceMenu === space.id}
                    <div class="absolute right-4 top-12 z-30 w-40 bg-white rounded-xl border border-[#e5e7eb] shadow-lg py-1 overflow-hidden">
                      <button onclick={() => { goto(`/dashboard/venues/${venue.id}/spaces/${space.id}`); activeSpaceMenu = null; }}
      ...
      <div class="sm:hidden divide-y divide-[#f9fafb]">
        {#each venue.spaces as space}
          <div class="px-4 py-4 relative">
            <a href="/dashboard/venues/{venue.id}/spaces/{space.id}" class="block">
              <div class="pr-8">
