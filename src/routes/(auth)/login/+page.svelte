<script lang="ts">
  ...
  * The server will verify userRole is Admin or Developer in Firestore.
   */
  async function exchangeTokenForSession(credential: UserCredential): Promise<void> {
    ...
    authStore.setUser({
      ...
      role: data.user!.role as 'super_admin' | 'admin' | 'support' | 'developer',
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      if (msg.includes('popup-closed-by-user') || msg.includes('cancelled-popup-request')) {
        ...
        error = 'Access denied. This Google account does not have dashboard privileges.';
      } else {
