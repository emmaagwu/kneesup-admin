 ...
 *  • Server verifies the token, checks Firestore for userRole === "Admin",
...
export async function createAdminSession(idToken: string): Promise<{
  ...
  const user: AdminUser = {
    ...
    role: (d['adminRole'] as AdminUser['role']) ?? 'admin',
 ...
 * Returns null if the cookie is missing, expired, or the user is not Admin.
 */
